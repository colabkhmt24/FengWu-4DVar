    ...
    def __init__(
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution, dim=dim // 2, norm_layer=norm_layer
            )
        else:
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer_up = BasicLayer_up(
                ...
                window_size=window_size,
                input_resolution=[
                    img_size[0] // stride[0] // 2 ** (self.num_layers - 1 - i_layer),
                    img_size[1] // stride[1] // 2 ** (self.num_layers - 1 - i_layer),
                ],
                mlp_ratio=self.mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
...
class Enc_net(nn.Module):
    def __init__(
        ...
        patch_size=[2, 2],
        stride=[2, 2],
        inchans_list=[6, 13, 13, 13, 13, 13, 4, 13, 13, 13, 13, 13],
        embed_dim=96,
        lgnet_embed=768,
        depths=[2, 2, 2],
        num_heads=[3, 6, 12],
        ...
        for i in range(len(self.inchans_list) // inp_length):
            if inp_length == 1:
                ...
                inchans = (
                    self.inchans_list[i]
                    + self.inchans_list[len(self.inchans_list) // 2 + i]
                )
            else:
                raise ValueError
        ...
        self.proj = nn.Linear(
            embed_dim * len(self.inchans_list) // inp_length * 2 ** (len(depths) - 1),
            lgnet_embed,
        )
    ...
    def forward(self, x):
        ...
        for i in range(len(self.inchans_list) // self.inp_length):
            if self.inp_length == 1:
                ...
                data = torch.cat(
                    (data_split[i], data_split[len(self.inchans_list) // 2 + i]), dim=1
                )
...
class Dec_net(nn.Module):
    def __init__(
        ...
        for i in range(len(self.outchans_list)):
            ...
            self.final_proj_list.append(
                nn.ConvTranspose2d(
                    in_channels=embed_dim,
                    out_channels=self.outchans_list[i],
        ...
        self.proj = nn.Linear(
            lgnet_embed,
            embed_dim * 2 ** (len(self.depths) - 1) * len(self.outchans_list),
        )

    def forward(self, x, downsample_list):
        data_proj = self.proj(x)
        data_split = torch.split(
            data_proj, self.embed_dim * 2 ** (len(self.depths) - 1), dim=-1
        )
        out_data_mean_list = []
        out_data_std_list = []
        for i in range(len(self.outchans_list)):
            out_data = self.dec_list[i](data_split[i], downsample_list[i]).permute(
                0, 3, 1, 2
            )
...
class LG_net(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(
                1,
                img_size[0] // patch_size[-2] * (img_size[1] // patch_size[-1]),
                embed_dim,
            )
        )
...
class LGUnet_all(nn.Module):
    def __init__(
        ...
        lg_patch_size = [
            stride[i] * 2 ** (len(enc_depths) - 1) for i in range(len(stride))
        ]
        self.net = LG_net(
            img_size=img_size,
